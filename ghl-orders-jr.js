...
// Cuenta "entradas" a partir del monto: ajusta si tus tickets no son de $1299 MXN c/u.
...
module.exports = async (req, res) => {
  ...
  if (!GHL_TOKEN) {
    res.status(500).json({ error: "Falta GHL_API_KEY en las variables de entorno de Vercel" });
    return;
  }
  ...
  try {
    ...
    const enriched = await mapLimit(newOrders, 8, async (o) => {
      ...
      // Nombre del/los producto(s) de la orden — se usa en el front para saber a qué
      // evento pertenece esta venta (ej. "Boleto CDMX 16 Agosto").
